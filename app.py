        ...
        if not pelit:
            for script in soup.find_all("script", type="application/ld+json"):
                try:
                    ...
                    for ev in events:
                        if ev.get("@type") not in ("SportsEvent", "Event"): 
                            continue
                        start = ev.get("startDate", "")
                        if not any(pf in start for pf in paiva_formaatit): 
                            continue
                            
                        time_m = re.search(r'T(\d{2}:\d{2})', start)
                        aika = time_m.group(1) if time_m else "??"
                        koti = ((ev.get("homeTeam") or {}).get("name") or ev.get("performer", {}).get("name", "Jokerit"))
