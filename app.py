...
<style>
    ...
    /* Tapahtumakortit - päänäytäntö */
    .ev-main {
        color: #f0f0f0; font-size: 21px; margin-bottom: 14px;
        line-height: 1.4; border-left: 3px solid #5bc0de; padding-left: 10px;
    }
    /* Tapahtumakortit - baletti */
    .ev-ballet {
        color: #d0c8ff; font-size: 20px; margin-bottom: 12px;
        line-height: 1.4; border-left: 3px solid #9b8fd4; padding-left: 10px;
    }
    /* Tapahtumakortit - perhe/lasten */
    .ev-family {
        color: #b0c8b0; font-size: 18px; margin-bottom: 10px;
        line-height: 1.3; border-left: 2px solid #607860; padding-left: 10px;
    }
    /* Tapahtumakortit - piensali */
    .ev-small {
        color: #999; font-size: 16px; margin-bottom: 8px;
        line-height: 1.2; border-left: 2px solid #444; padding-left: 8px;
    }
...
```
            # Muunna aikamerkkijono vertailukelpoiseksi
            dt_laiva = None
            if aika_str:
                try:
    ...
    # Järjestä ajan mukaan, ota seuraavat 3
...
“””
Palauttaa CSS-luokan tapahtuman tärkeyden mukaan.
Järjestys: ooppera > muu päänäyttämö > baletti > perhe/lasten > piensali
“””
n = nimi.lower()
l = loc_name.lower()
...
```
if any(kw in n for kw in pieni_keywords) or any(kw in l for kw in pieni_keywords):
    return "ev-small"
if any(kw in n for kw in perhe_keywords):
    return "ev-family"
if any(kw in n for kw in baletti_keywords):
    return "ev-ballet"
if any(kw in n for kw in ooppera_keywords):
    return "ev-main"   # Ooppera = korkein prioriteetti
return "ev-main"
```
...
```
            ...
            # Rakenna HTML tapahtuman tärkeysluokan mukaan
            if css_class == "ev-main":
                html_block = (
                    f"<div class='ev-main'>"
                    f"► <b>Klo {aika_naytto}</b>: {nimi}<br>"
                    f"<span class='ev-venue-tag'>📍 {sali_info}</span>"
                    f"</div>"
                )
            elif css_class == "ev-ballet":
                html_block = (
                    f"<div class='ev-ballet'>"
                    f"► <b>Klo {aika_naytto}</b>: {nimi}<br>"
                    f"<span class='ev-venue-tag' style='color:#c8b8ff;'>📍 {sali_info}</span>"
                    f"</div>"
                )
            elif css_class == "ev-family":
                html_block = (
                    f"<div class='ev-family'>"
                    f"▷ Klo {aika_naytto}: {nimi}<br>"
                    f"<span class='ev-venue-small'>📍 <i>{sali_info}</i></span>"
                    f"</div>"
                )
            else:  # ev-small
                html_block = (
                    f"<div class='ev-small'>"
                    f"▷ Klo {aika_naytto}: {nimi}<br>"
                    f"<span class='ev-venue-small'>📍 <i>{sali_info}</i></span>"
                    f"</div>"
                )
            # Lisätään listaan – ooppera ensin, baletti toiseksi, muut lopussa
...
“””
Hakee Jokereiden otteluohjelman jokerit.fi/ottelut -sivulta.
Palauttaa listan diktejä: {aika, koti, vieras, venue, tournament}
“””
pelit = []
dt = datetime.datetime.strptime(pvm_iso, “%Y-%m-%d”)
# Muodosta pvm-vertailuformaatit suomalaisia sivuja varten
paiva_formaatit = [
dt.strftime(”%-d.%-m.%Y”),   # 5.3.2025
dt.strftime(”%d.%m.%Y”),      # 05.03.2025
dt.strftime(”%-d.%-m”),        # 5.3
dt.strftime(”%d.%m”),          # 05.03
dt.strftime(”%Y-%m-%d”),       # 2025-03-05 (JSON/data-attribuutit)
]
tunnetut_veniat = {
...
try:
    ...
    # Strategia 1: Etsi <tr> tai <div> elementtejä, joissa on oikea päivämäärä
    for elem in soup.find_all(["tr", "div", "li", "article"]):
        elem_text = elem.get_text(separator=" ", strip=True)
        if not any(pf in elem_text for pf in paiva_formaatit):
            continue
        # Aika (HH:MM)
        ...
        # Joukkueet – etsitään "Jokerit" tai muoto "A – B" tai "A vs B"
        team_m = re.search(
            r'(Jokerit|[A-ZÄÖÅ][a-zäöå\-\.]{2,}(?:\s[A-ZÄÖÅ][a-zäöå]{1,})?)'
            r'\s*[-–vs]+\s*'
            r'([A-ZÄÖÅ][a-zäöå\-\.]{2,}(?:\s[A-ZÄÖÅ][a-zäöå]{1,})?)',
            elem_text
        )
        ...
        # Venue
        venue_label = "Veikkaus Arena"  # oletus Jokereiden kotipaikka
    ...
    # Strategia 2: JSON/LD-data sivulta (script-tagit)
    if not pelit:
        for script in soup.find_all("script", type="application/ld+json"):
            try:
                ...
                for ev in events:
                    ...
                    if not any(pf in start for pf in paiva_formaatit): continue
                    time_m = re.search(r'T(\d{2}:\d{2})', start)
                    aika = time_m.group(1) if time_m else "??"
                    koti = ((ev.get("homeTeam") or {}).get("name") or
                            ev.get("performer", {}).get("name", "Jokerit"))
                    vieras = (ev.get("awayTeam") or {}).get("name", "?")
                    venue_name = (ev.get("location") or {}).get("name", "Veikkaus Arena")
                    venue_short = "Veikkaus Arena"
                    for kw, (lbl, short) in tunnetut_veniat.items():
                        if kw in venue_name.lower():
                            venue_short = short
                            break
                    pelit.append({
                        "aika": aika,
...
# ERIKOISOTTELUN TUNNISTUS (playoffs, derby…)

# ==========================================

def tunnista_erikoispeli(koti: str, vieras: str, tournament: str = “”) -> list:
“””
Palauttaa listan badge-merkkijonoista erikoistapahtumia varten.
“””
tags = []
kl = koti.lower()
vl = vieras.lower()
...
```
helsinki_joukkueet = ["hifk", "jokerit", "kiekko-espoo", "blues", "kiekko-vantaa"]
helsinki_osumia = sum(1 for j in helsinki_joukkueet if j in kl or j in vl)
if helsinki_osumia >= 2:
    tags.append("<span class='badge-fire'>🔥 PAIKALLISPELI</span>")

if any(kw in tl for kw in ["playoff", "pudotus"]):
    tags.append("<span class='badge-gold'>🏆 PLAYOFFS</span>")
elif any(kw in tl for kw in ["cup", "kansallinen"]):
    tags.append("<span class='badge-gold'>🥇 CUP</span>")
elif any(kw in tl for kw in ["champions"]):
    tags.append("<span class='badge-gold'>⭐ CHAMPIONS LEAGUE</span>")
elif any(kw in tl for kw in ["mm", "world"]):
    tags.append("<span class='badge-gold'>🌍 MM-OTTELU</span>")
...
pelit.append({
“koti”: koti, “vieras”: vieras,
“aika”: aika, “tournament”: tournament
})
except: pass
return pelit
...
for p in paikat:
    ...
    if "hifk" in nimi_lower:
        ...
        for peli in omat:
            ...
            tapahtumat_html.append(
                ...
                f"{'<br>' + tag_str if tag_str else ''}"
                f"</div>"
            )
        if tapahtumat_html:
            p["lopetus_html"] = (
                f"<div style='margin-top:10px;'>"
                f"<span class='live-event'>KOTIOTTELU TÄNÄÄN – Nordis:</span><br>"
                f"{''.join(tapahtumat_html)}"
                f"<span style='color:#ccc;font-size:17px;display:block;margin-top:4px;'>"
                f"ℹ️ Yleisö purkautuu n. 2,5h aloituksesta.</span>"
                f"</div>"
            )
        else:
            p["lopetus_html"] = "<span class='no-event'>Ei Liiga-kotiottelua tänään.</span>"
    ...
    # ---------- Kiekko-Espoo ----------
        ...
        if tapahtumat_html:
            p["lopetus_html"] = (
                f"<div style='margin-top:10px;'>"
                f"<span class='live-event'>KOTIOTTELU TÄNÄÄN – Metro Areena:</span><br>"
                f"{''.join(tapahtumat_html)}"
                f"<span style='color:#ccc;font-size:17px;display:block;margin-top:4px;'>"
                f"ℹ️ Yleisö purkautuu n. 2,5h aloituksesta.</span>"
                f"</div>"
    ...
    # ---------- Jokerit (Nordis + Veikkaus Arena + Kerava) ----------
        ...
        if jokerit_pelit:
            for peli in jokerit_pelit:
                ...
                tapahtumat_html.append(
                    f"<div style='color:#d0d0d0; font-size:21px; margin-bottom:8px;'>"
                    f"► <b>Klo {peli['aika']}</b>: {peli['koti']} – {peli['vieras']}<br>"
                    f"{venue_badge}"
                    f"{'<br>' + tag_str if tag_str else ''}"
                    f"</div>"
                )
            p["lopetus_html"] = (
                f"<div style='margin-top:10px;'>"
                f"<span class='live-event'>KOTIOTTELU TÄNÄÄN – Jokerit:</span><br>"
                f"{''.join(tapahtumat_html)}"
                f"<span style='color:#ccc;font-size:17px;display:block;margin-top:4px;'>"
                f"ℹ️ Yleisö purkautuu n. 2,5h aloituksesta.</span>"
                f"</div>"
            ...
            p["lopetus_html"] = (
                f"<span class='no-event'>Ei Jokereiden ottelua tänään (tai scraper ei löytänyt).</span><br>"
...
with tab2:
    urheilu = [
        {"nimi": "HIFK – Nordis (Helsingin Jäähalli)", "kap": "8 200 hlö",
         "linkki": "https://hifk.fi/"},
        {"nimi": "Kiekko-Espoo – Metro Areena", "kap": "8 500 hlö",
         "linkki": "https://kiekko-espoo.com/"},
        {"nimi": "Jokerit – Veikkaus Arena / Nordis / Kerava", "kap": "2 000–13 500 hlö",
...
with tab3:
    messut = [
        {"nimi": "Messukeskus", "kap": "Jopa 50 000 hlö",
         "lopetus_html": (
             "<span style='color:#d0d0d0; font-size:20px;'>"
             "Poistumapiikki tyypillisesti klo 16–18. Tarkista erikoistapahtumat.</span>"
         ),
         "linkki": "https://messukeskus.com/tapahtumakalenteri/"},
        {"nimi": "Tavastia & Kaapelitehdas", "kap": "900–3 000 hlö",
         "lopetus_html": (
             "<span style='color:#d0d0d0; font-size:20px;'>"
             "Musiikkikeikat loppuvat yleensä klo 23:00–23:30. Katso sivut.</span>"
         ),
         "linkki": "https://tavastiaklubi.fi/"},
        {"nimi": "Olympic Stadium / Suurkonsertit", "kap": "~40 000 hlö",
         "lopetus_html": (
             "<span style='color:#ffeb3b; font-size:20px; font-weight:bold;'>"
             "⭐ Tarkista tulevat stadionkonsertit! Poistumapiikki heti keikan jälkeen.</span>"
         ),
